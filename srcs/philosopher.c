...
/*   Updated: 2024/11/07 22:24:45 by lmonsat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void routine_condition(struct s_philosopher *philosophe, struct s_data_shared *data)
{
	...
	{
		pthread_mutex_lock(&data->lock_eat_assign);
		philosophe->has_eaten = 1;
		philosophe->nb_plate_eaten++;
		pthread_mutex_unlock(&data->lock_eat_assign);
	...
	{
		pthread_mutex_lock(&data->lock_sleep_assign);
		...
		pthread_mutex_unlock(&data->lock_sleep_assign);
	}
	...
	{
		pthread_mutex_lock(&data->lock_think_assign);
		philosophe->has_thought = 1;
		printf("{%u ms} philosophe[%d] think\n", get_time(0), philosophe->id);
		pthread_mutex_unlock(&data->lock_think_assign);
	}
...
/* vérifier l'adresse des fourchettes
	print console après que un philosopher ait mourr
